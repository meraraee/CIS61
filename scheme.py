...
def scheme_eval(expr, env):
    ...
    if (scheme_symbolp(first) # first might be unhashable
        ...
        return scheme_eval(expr, env) # last item gets evaluated (do_begin_form)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    converted_args = []
    while args != nil:
        converted_args.append(args.first)
        args = args.second
    if procedure.use_env:
        converted_args.append(env)
    try:
        return procedure.fn(*converted_args)
    except TypeError:
...
def do_lambda_form(vals, env):
    ...
    check_form(vals, 2) # checks valid form of func
    formals = vals[0]   # indicates parameters
    check_formals(formals)  # checks valid parameters
...
# returning unevaluated expressions
def do_quote_form(vals):
    """Evaluate a quote form with parameters VALS."""
    check_form(vals, 1, 1)
    return vals.first # q B6
...
def do_begin_form(vals, env):
    ...
    while vals.second is not nil:
        ...
        vals = vals.second
    return vals.first  #  last item in vals is unevaluated; already gets evaluated in scheme_eval
